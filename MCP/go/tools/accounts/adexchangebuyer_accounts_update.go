...
func Adexchangebuyer_accounts_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAdexchangebuyer_accounts_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_accounts_id",
		mcp.WithDescription("Updates an existing account."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Adexchangebuyer_accounts_updateHandler(cfg),
	}
}
